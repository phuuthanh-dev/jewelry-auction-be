...
public interface AuctionRegistrationRepository extends JpaRepository<AuctionRegistration, Integer> {
    ...
    @Query("SELECT ar FROM AuctionRegistration ar JOIN FETCH ar.auction a WHERE a.id = :auctionId")
    List<AuctionRegistration> findByAuctionIdAndValid(@Param("auctionId") Integer auctionId);
